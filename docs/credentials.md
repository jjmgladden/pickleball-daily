...
| **`ANTHROPIC_API_KEY`** | API key (Anthropic) | ✅ Active (all three locations) | 2026-04-26 (Session 8); rotated same day | `worker/src/index.js` (`/ai` route) + local test scripts | Local `.env` + GitHub Secret + Cloudflare Worker secret | [console.anthropic.com → API keys](https://console.anthropic.com/settings/keys) |
...
- **Storage:**
  ...
  - Cloudflare Worker secret (✅ Active 2026-04-27 — set via Cloudflare dashboard after `wrangler secret put` paste was mangled by Windows cmd terminal)
...
- **Rotation history:**
  - 2026-04-26 (Session 8) — initial creation as `pickleball-daily-prod`. Pasted into local `.env` + GitHub Secret.
  - 2026-04-26 (Session 8, ~minutes later) — rotated; original key value was accidentally exposed in plaintext in the chat transcript via mistyped wrangler argument. Owner deleted exposed key + created replacement with same name. Distributed to LastPass + local `.env` + GitHub Secret.
  - 2026-04-27 (Session 8) — same key (no value change) finally landed in Cloudflare Worker secret store via dashboard paste, after `wrangler secret put` paste was mangled by Windows cmd terminal masked-input field (only captured 1 character). Smoke test verified end-to-end with first real Anthropic answer.
...
- **2026-04-26 (Session 8)** — `ANTHROPIC_API_KEY` rotated within minutes of original creation. Original key value was accidentally exposed in plaintext in this session's terminal/transcript (mistyped as the secret-name argument to `wrangler secret put` instead of the secret-value prompt). Owner deleted the exposed key at console.anthropic.com and created a replacement with the same name `pickleball-daily-prod`. New value redistributed to LastPass + local `.env` + GitHub Secret. Cloudflare Worker secret set later via wrangler when deploy succeeded.
- **2026-04-27 (Session 8)** — Cloudflare Worker `pickleball-daily-api` deployed first time at `https://pickleball-daily-api.jjmgladden.workers.dev` (subdomain `jjmgladden.workers.dev` registered as part of this deploy — permanent for the Cloudflare account, will host all future Workers across all projects). `ANTHROPIC_API_KEY` set as Worker secret via `wrangler secret put`. Initial smoke tests returned Anthropic 400 with empty body. Diagnosis via Node `scripts/test-anthropic.js` (direct call with same key + model + payload) returned 200 OK — proving key + model + payload were all valid; bug had to be Worker-runtime-specific. Added temporary `DEBUG-AI` logging to Worker that revealed `keyLen=1 keyPrefix=▬` — Windows cmd terminal had mangled the masked-input paste during `wrangler secret put`, capturing only one character. Recovery: re-pasted the verified-working key via Cloudflare dashboard (browser-based input handles paste correctly) at Workers & Pages → pickleball-daily-api → Settings → Variables and Secrets → ANTHROPIC_API_KEY → Rotate → paste → Deploy. Smoke test then succeeded; first real Anthropic answer returned (~$0.008 charged). KB-0008 closed.
- **2026-04-27 (Session 8)** — Wrangler OAuth token expired and self-refresh failed silently when owner clicked "Cancel" on the unprompted Cloudflare consent popup (advised by Claude — wrong call; the popup WAS the refresh attempt). Recovery via fresh `wrangler login` from `worker/`. Lesson recorded: future unprompted Cloudflare consent popups during wrangler-related work should be Authorized, not cancelled.
